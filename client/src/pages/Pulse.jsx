...
export default function Pulse({ onDataLoad }) {
  ...
  // Only real bank accounts (positive or explicitly added), no One Sept-type
  const bankAccounts = (d.accounts || []).filter(a =>
    a.balance !== 0 || (a.name || '').toLowerCase().includes('permata') ||
    (a.name || '').toLowerCase().includes('bca') ||
    (a.name || '').toLowerCase().includes('bni') ||
    (a.name || '').toLowerCase().includes('helm care')
  ).slice(0, 5)
    ...
    <div className="page">
      ...
      <div style={{ padding: '14px 16px 10px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
          <div style={{ width: 28, height: 28, borderRadius: '50%', background: '#B5D4F4', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 11, fontWeight: 600, color: '#0C447C', cursor: 'pointer' }}
      ...
      <div style={{ margin: '0 16px 12px', background: st.bg, borderRadius: 24, padding: '18px 18px 16px', position: 'relative', overflow: 'hidden' }}>
        ...
        <div style={{ fontSize: 11, color: 'rgba(255,255,255,.65)', marginBottom: 12, lineHeight: 1.4 }}>
          Runway {d.runway} days · {d.aiText?.split('.')[0]}
        </div>

        {/* Account chips — only real banks */}
        {bankAccounts.length > 0 && (
          <div style={{ display: 'flex', gap: 5, flexWrap: 'wrap', marginBottom: 12 }}>
            {bankAccounts.map(a => (
              <div key={a.id} style={{
                background: a.balance < 0 ? 'rgba(0,0,0,.2)' : 'rgba(255,255,255,.15)',
                border: '0.5px solid rgba(255,255,255,.2)',
                borderRadius: 20, padding: '3px 10px', fontSize: 10, color: '#fff'
              }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8 }}>
          <button onClick={() => setShowAnalysis(true)} style={{
            background: 'rgba(255,255,255,.2)', border: '0.5px solid rgba(255,255,255,.35)',
            borderRadius: 14, padding: '9px 0', fontSize: 12, color: '#fff', cursor: 'pointer'
          }}>View analysis</button>
          <button onClick={() => setShowAnalysis(true)} style={{
            background: '#fff', border: 'none',
            borderRadius: 14, padding: '9px 0', fontSize: 12, fontWeight: 500,
      ...
      <div style={{ margin: '0 16px 10px', background: 'var(--bg-2)', borderRadius: 16, padding: '12px 14px', border: '0.5px solid var(--border)' }}>
        ...
        {[
          { label: 'Cash', value: fmtFull(d.totalBalance), color: d.totalBalance >= 0 ? 'var(--green)' : 'var(--red)' },
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', padding: '6px 0 0' }}>
          ...
          <span style={{ fontSize: 15, fontWeight: 500, color: d.netPosition >= 0 ? 'var(--green)' : 'var(--red)' }}>
            {d.netPosition >= 0 ? '+' : ''}{fmtFull(d.netPosition)} IDR
          </span>
        </div>
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8, margin: '0 16px 12px' }}>
        <div style={{ background: '#DCFCE7', borderRadius: 16, padding: '12px 14px' }}>
          ...
          <div style={{ fontSize: 10, color: '#16A34A', marginTop: 3 }}>
            {debts.filter(x => x.type === 'receivable').length} receivable
          </div>
        </div>
        <div style={{ background: '#FEE2E2', borderRadius: 16, padding: '12px 14px' }}>
          ...
          <div style={{ fontSize: 10, color: '#E24B4A', marginTop: 3 }}>
            {debts.filter(x => x.type === 'payable').length} payable
          </div>
        </div>
      </div>
              ...
              <div key={debt.id} style={{ margin: '0 16px 8px', background: 'var(--bg)', border: '0.5px solid var(--border)', borderRadius: 20, padding: '12px 14px' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                  ...
                  <div style={{ textAlign: 'right', flexShrink: 0 }}>
                    ...
                    <div style={{ fontSize: 9, padding: '2px 7px', borderRadius: 8, background: days < 0 ? '#FCEBEB' : days === 0 ? '#FCEBEB' : '#FAEEDA', color: days < 0 ? '#A32D2D' : '#633806', display: 'inline-block', marginTop: 3 }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 7, padding: '0 16px 24px' }}>
        {[
          { label: 'RUNWAY',       value: `${d.runway}d`,           sub: 'at current burn',  color: (d.runway||0) > 14 ? 'var(--green)' : (d.runway||0) > 7 ? '#D97706' : '#E24B4A', bar: Math.min(100, Math.max(5, (d.runway||0) * 3)), barColor: (d.runway||0) > 14 ? '#16A34A' : (d.runway||0) > 7 ? '#D97706' : '#E24B4A' },
          { label: 'BURN RATE',    value: `${fmt(d.burnRate)}/d`,   sub: '30-day avg',       color: 'var(--text)', bar: 40, barColor: '#38BDF8' },
          { label: 'EXPECTED IN',  value: `+${fmt(d.receivables)}`, sub: 'receivables',      color: '#16A34A', bar: d.receivables > 0 ? 60 : 0, barColor: '#16A34A' },
          { label: 'EXPECTED OUT', value: `-${fmt(d.payables)}`,    sub: 'payables',         color: d.payables > 0 ? '#D97706' : 'var(--text-3)', bar: d.payables > 0 ? 80 : 0, barColor: '#D97706' },
        ].map(v => (
      ...
      {/* Analysis Modal */}
      {showAnalysis && (
        <div onClick={() => setShowAnalysis(false)} style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,.45)', zIndex: 300, display: 'flex', alignItems: 'flex-end', justifyContent: 'center' }}>
          <div onClick={e => e.stopPropagation()} style={{ background: 'var(--bg)', borderRadius: '24px 24px 0 0', padding: '16px 18px 32px', width: '100%', maxWidth: 430 }}>
            <div style={{ width: 36, height: 4, background: 'var(--border-2)', borderRadius: 2, margin: '0 auto 14px' }} />
            ...
            {[
              { label: 'RUNWAY TREND', text: `${d.runway} days at ${fmt(d.burnRate)}/day burn rate.${d.runway < 0 ? ' Deepens by ' + fmt(d.burnRate) + ' daily without income.' : ''}` },
              ...
              { label: 'RECOMMENDATION', text: (d.runway||0) < 7 ? 'Collect receivables immediately. Delay non-critical expenses. Any incoming payment extends runway.' : (d.runway||0) < 14 ? 'Review upcoming payments. Confirm receivables today.' : 'Finances healthy. Focus on growing income.' },
            ].map(item => (
            ...
            <button onClick={() => setShowAnalysis(false)} style={{ width: '100%', padding: 13, borderRadius: 14, background: 'var(--text)', color: '#fff', border: 'none', fontSize: 13, fontWeight: 500, cursor: 'pointer', marginBottom: 8 }}>Close</button>
          </div>
        </div>
      )}

      {/* Pay / Mark Received Modal */}
      {payModal && (
        <div onClick={() => setPayModal(null)} style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,.45)', zIndex: 300, display: 'flex', alignItems: 'flex-end', justifyContent: 'center' }}>
          <div onClick={e => e.stopPropagation()} style={{ background: 'var(--bg)', borderRadius: '24px 24px 0 0', padding: '16px 18px 32px', width: '100%', maxWidth: 430 }}>
            <div style={{ width: 36, height: 4, background: 'var(--border-2)', borderRadius: 2, margin: '0 auto 14px' }} />
            ...
            <select value={payForm.account} onChange={e => setPayForm(p => ({ ...p, account: e.target.value }))}
              style={{ width: '100%', padding: '11px 13px', borderRadius: 14, border: '0.5px solid var(--border-2)', fontSize: 13, background: 'var(--bg-2)', color: 'var(--text)', marginBottom: 8 }}>
              <option value="">Select account</option>
              {(d.accounts || []).map(a => <option key={a.name} value={a.name}>{a.name} · {fmt(a.balance)}</option>)}
            </select>

            {payForm.amount && (
              <div style={{ background: 'var(--bg-2)', borderRadius: 12, padding: '9px 12px', marginBottom: 12, display: 'flex', justifyContent: 'space-between', border: '0.5px solid var(--border)' }}>
                <span style={{ fontSize: 12, color: 'var(--text-3)' }}>Remaining after</span>
                <span style={{ fontSize: 12, fontWeight: 500, color: payModal.amount - Number(payForm.amount) > 0 ? '#D97706' : '#16A34A' }}>
                  {fmt(Math.max(0, payModal.amount - Number(payForm.amount)))} IDR
                </span>
              </div>
            )}

            <button disabled={!payForm.amount || paying} onClick={handlePay} style={{
              width: '100%', padding: 13, borderRadius: 14, border: 'none', fontSize: 13, fontWeight: 500, cursor: 'pointer', marginBottom: 8,
              ...
              {paying ? 'Processing...' : Number(payForm.amount) >= Number(payModal.amount)
                ? (payModal.type === 'receivable' ? 'Mark as received · ' : 'Pay in full · ') + fmt(Number(payForm.amount)) + ' IDR'
                : (payModal.type === 'receivable' ? 'Mark ' : 'Pay ') + fmt(Number(payForm.amount)) + ' IDR'}
            </button>
            <button onClick={() => setPayModal(null)} style={{ width: '100%', padding: 11, borderRadius: 14, background: 'none', color: 'var(--text-3)', border: '0.5px solid var(--border)', fontSize: 13, cursor: 'pointer' }}>Cancel</button>
          </div>
        </div>
      )}
      ...
      {/* Snooze Modal */}
      {snoozeModal && (
        <div onClick={() => setSnoozeModal(null)} style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,.45)', zIndex: 300, display: 'flex', alignItems: 'flex-end', justifyContent: 'center' }}>
          <div onClick={e => e.stopPropagation()} style={{ background: 'var(--bg)', borderRadius: '24px 24px 0 0', padding: '16px 18px 32px', width: '100%', maxWidth: 430 }}>
            <div style={{ width: 36, height: 4, background: 'var(--border-2)', borderRadius: 2, margin: '0 auto 14px' }} />
            <div style={{ fontSize: 17, fontWeight: 500, color: 'var(--text)', marginBottom: 3 }}>
              {snoozeModal.type === 'receivable' ? 'Send reminder' : 'Snooze reminder'}
            </div>
            <div style={{ fontSize: 12, color: 'var(--text-3)', marginBottom: 16 }}>
              {snoozeModal.counterparty} · {fmt(snoozeModal.amount)} IDR
            </div>
            ...
            <div style={{ background: 'rgba(14,165,233,.08)', border: '0.5px solid rgba(14,165,233,.25)', borderRadius: 14, padding: '10px 12px', marginBottom: 14 }}>
              <div style={{ fontSize: 11, color: '#0369A1', lineHeight: 1.5 }}>
                Snoozing 3 days deepens deficit by ~{fmt((d.burnRate || 0) * 3)} IDR.
              </div>
            </div>

            <button onClick={() => setSnoozeModal(null)} style={{ width: '100%', padding: 11, borderRadius: 14, background: 'none', color: 'var(--text-3)', border: '0.5px solid var(--border)', fontSize: 13, cursor: 'pointer' }}>Cancel</button>
          </div>
        </div>
      )}
    </div>
