...
> Restore (only if needed later): `psql $PGCONN -f "prod_backup_pre043_<stamp>.sql"`.
...
-- 5) no Telegram routing rows yet (fresh table) + no existing data changed
SELECT count(*) AS telegram_state_rows FROM telegram_user_state;          -- expect 0
-- (existing tables untouched — 043 only CREATEs; compare counts to pre-apply if desired)
SELECT count(*) AS users_count FROM users;                                -- unchanged vs pre-apply
SELECT count(*) AS businesses_count FROM businesses;                      -- unchanged vs pre-apply
```
...
users/businesses counts unchanged. STOP and do NOT flip the flag if any check fails.
...
3. **Do NOT drop `telegram_user_state`** unless explicitly needed — it's additive, inert
   ...
   teardown is truly required: `DROP TABLE IF EXISTS telegram_user_state;
   DROP FUNCTION IF EXISTS fn_telegram_user_state_set_updated_at();` (pre-cleanup) — or
   restore the Step-1 pg_dump.
